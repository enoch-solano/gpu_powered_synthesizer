...
namespace Additive {
    ...
    float* &h_tmp_buffer, float* &h_buffer, float* &h_adsr, bool* &h_v_ons, int num_samples, int num_voices, int num_harms);
    ...
    void my_v_compute(float *buffer, float angle, float* h_buffer, float* h_v_gains, float2* h_freq_gains, float* h_adsr, bool* h_v_ons, int numSamples, int numSinusoids, int numVoices);
